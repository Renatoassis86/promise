...
-- E-mails hardcoded pra bater com ADMIN_EMAILS em src/lib/adminAuth.ts; atualize os dois juntos.
...
CREATE POLICY "Admin le todas as pre-matriculas" ON public.pre_matriculas
  FOR SELECT TO authenticated USING ((auth.jwt() ->> 'email') IN ('calebe@promiseenglish.com', 'renato.consultoria@cidadeviva.org'));
