import { createClient } from "@/lib/supabase/server";
import ApresentacaoPlanoDeNegocio from "@/components/admin/ApresentacaoPlanoDeNegocio";
import { APRESENTACAO_SLIDES } from "@/lib/apresentacaoSlides";
import { calcularProjecaoFinanceira, type LinhaFinanceira, type Frente, type TipoLinha, type ModoLinha } from "@/lib/financasCalculo";

export default async function ApresentacaoPlanoDeNegocioPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  const userEmail = user!.email!;

  const [{ data: linhasRows }, { data: configRow }] = await Promise.all([
    supabase.from("plano_financas_linhas").select("*").eq("user_email", userEmail),
    supabase.from("plano_financas_config").select("*").eq("user_email", userEmail).maybeSingle(),
  ]);

  const linhas: LinhaFinanceira[] = (linhasRows ?? []).map((row) => ({
    id: row.id,
    tipo: row.tipo as TipoLinha,
    modo: row.modo as ModoLinha,
    macroArea: row.macro_area,
    rubrica: row.rubrica,
    frente: (row.frente as Frente) ?? null,
    ticketMedio: row.ticket_medio != null ? Number(row.ticket_medio) : null,
    reajusteTicketPct: row.reajuste_ticket_pct != null ? Number(row.reajuste_ticket_pct) : 0,
    percentualReceitaPct: row.percentual_receita_pct != null ? Number(row.percentual_receita_pct) : null,
    valoresPorAno: Object.fromEntries(Object.entries(row.valores_por_ano ?? {}).map(([ano, v]) => [Number(ano), Number(v)])),
    ordem: row.ordem ?? 0,
  }));

  const resultadoFinanceiro = calcularProjecaoFinanceira(linhas, Number(configRow?.investimento_inicial ?? 0));

  const temPremissasPreenchidas = linhas
    .filter((l) => l.tipo === "receita")
    .some((l) => Object.values(l.valoresPorAno).some((v) => (v ?? 0) > 0));

  return (
    <ApresentacaoPlanoDeNegocio
      slides={APRESENTACAO_SLIDES}
      resultadoFinanceiro={resultadoFinanceiro}
      temPremissasPreenchidas={temPremissasPreenchidas}
    />
  );
}
